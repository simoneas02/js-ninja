(function() {
  ...
  */
  ...
  - ^ --> fora de uma lista serve pra validar com o que inicia a string --> texto.match(/^</g);
  - $ --> serve pra validar com o que finaliza a string --> texto.match(/>$/g) --> pega todos os carcteres
  - m --> é uma flag pra ler mais de uma linha, geralmente é mais utilizado com símbolo de início de linha --> texto.match(/^</gm);
  - ? -->  representa um ou zero caracter que vir antes dele --> texto.match(/\n/g);
  - *? ou +? -->  faz uma captura não gulosa, ou seja faz o match até encontrar o caracter do próximo grupo --> texto.match(/(<\w+)(.+?)(<\/\w+>)/g);
  - (?:) --> usamos no início de uma expressão onde queremos apenas agrupar sem fazer a captura --> texto.match(/ju(?:n|l)ho/g);
  - texto.match(/<(\w+).+<\/(\1)>/g) --> exemplo match pela captura do agrupamento, fazendo referência dentro da regex <3, use sempre \nº_da_captura
  
  Métodos uasdos com Regex:
  - match --> retorna um array com a qtd de matchs encontrados ou null se não encontrar nada
  - replace(regex, string|function(capturas_do_regex))
  - split --> quebra a string passada em um array tendo com referência o valor passado como parâmetro --> '123-456-986'.split('-') // [ '123', '456', '986' ]
              '123.456.986-12'.split(/\D/g) // [ '123', '456', '986', '12' ] --> quebra os valores em array através da regex passada, no exemplo usei pelo valor que não é número /\D/ <3
  - search --> faz uma busca na string e retorna o primeiro index do valor encontrado -->  '123.456.986-12'.search(/\./); // 3
  
  Objeto RegExp()
  - cria um novo objeto com uma expressão regular do valor repassado -->  var regex = new RegExp('mone'); // /mone/ --> 'simone'.match(regex); //[ 'mone', index: 2, input: 'simone' ]
  - pode receber como parâmetro 
    - uma string --> var regex = new RegExp('mone')
    - uma regex --> var regex = new RegExp('\\d') --> // /\d/ --> tem que adicionar uma barra a mais para escapar a regex
    - aceita um segundo parâmetro que seria a flag --> var regex = new RegExp('\\d', 'g') --> 'simo123ne'.match(regex); // [ '1', '2', '3' ] --> faz um match com todos os números da string passada
  - use \ para escapar caracteres
  Métodos da regex
  - regex.test(string) --> retorna true se encontrar o valor repassado como parâmetro na regex ou falso caso não seja encontado --> regex.test('123') // true
  - regex.exec(string) --> a cada execução passa por cada index fazendo o match com a string passada, caso o valor seja encontrado retorna o valor + index + string passada, caso contrário, retorna null, lembrando que tem que passar o parâmetro global 'g'
    - var regex = /\d/g; var name = 'simone123'; var result;
      - regex.exec(name); // [ '1', index: 6, input: 'simone123' ]
      - regex.exec(name); // [ '2', index: 7, input: 'simone123' ]
      - regex.exec(name); // [ '3', index: 8, input: 'simone123' ]
      - while( result = regex.exec(name) !== null ) { console.log(result) }
        - true
        - true
        - true
