(function() {
  ...
  */
  function isOperatorValid( param ) {
    return !!operation[param] ? true : false;
  }
  ...
  */
  function showOperationMessage( param, x, y ) {
    return `A operação ${x} ${param} ${y} =`;
  }
  ...
  */
  operationSignal = 'x';
  const notValid = calculator( operationSignal );

  if( notValid ) {
    number1 = 10;
    number2 = 5;

    console.log( showOperationMessage( operationSignal, number1, number2 ), notValid( number1, number2 ));
  } 
  else {
    console.log( showErrorMessage(operationSignal) );
  }
